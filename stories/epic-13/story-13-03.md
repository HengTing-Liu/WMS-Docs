...
实现效期预警功能，当物料效期低于阈值时自动发送预警通知。

---

## 2. 业务背景

医疗器械有严格的效期管理要求，过期物料不能使用。效期预警帮助仓库管理员及时处理临期物料。
...
| 条件 | 默认值 | 说明 |
|------|--------|------|
| expireDate - today <= threshold | 30天 | 效期低于阈值 |
| isEnabled = 1 | - | 物料启用 |
| qcStatus = PASSED | - | 质检合格 |
